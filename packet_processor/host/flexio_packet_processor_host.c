	...
	struct app_transfer_wq *sq_transf, int index, size_t zero_copy)
{
	...
	queue->sq_data_reuses_rq = zero_copy;
	if (zero_copy == 1) {
		// tmp trick code
		...
		queue->sqd_mkey = create_dpa_mkey(app_ctx, queue->sq_transf.wqd_daddr);
		if (!queue->sqd_mkey) {
 ...
 * app_ctx - app_ctx - pointer to app_context structure.
 */
static int create_app_sq(struct app_context *app_ctx, struct thread_context* thd_ctx, size_t use_copy)
{
	...
	for (int i = 0; i < thd_ctx->num_queues; i++) {
		...
		*/
		...
		if (sq_mem_alloc(app_ctx, thd_ctx, app_fp, &thd_ctx->queues[i].sq_transf, i, zero_copy)) {
 ...
 * app_ctx - app_ctx - pointer to app_context structure.
 */
static int create_app_rq(struct app_context *app_ctx, struct thread_context* thd_ctx)
{
	/* Pointer to the application Flex IO process (ease of use). */
	struct flexio_process *app_fp = app_ctx->flexio_process;
 ...
 * app_ctx - app_ctx - pointer to app_context structure.
 */
static int copy_sch_data_to_dpa(struct app_context *app_ctx, struct thread_context *thd_ctx, int buffer_location, size_t use_copy)
{
	/* Size of application information struct. */
	uint64_t struct_bsize = sizeof(struct host2dev_packet_processor_data2);
	/* Temporary application information struct to copy. */
	struct host2dev_packet_processor_data2 *h2d_data;
	/* Function return value. */
	int ret = 0;
	...
	h2d_data->threads_num = threads_num;
 ...
 * app_ctx - app_ctx - pointer to app_context structure.
 */
static int copy_thd_data_to_dpa(struct app_context *app_ctx, struct thread_context *thd_ctx, int buffer_location, size_t use_copy)
{
	/* Size of application information struct. */
	uint64_t struct_bsize = sizeof(struct host2dev_packet_processor_data1);
	/* Temporary application information struct to copy. */
	struct host2dev_packet_processor_data1 *h2d_data;
	/* Function return value. */
	int ret = 0;
...
static int clean_up_app_rq(struct app_context* app_ctx, struct thread_context *thd_ctx)
{
	...
	for (int i = 0; i < thd_ctx->num_queues; i++) {
		...
		if (!thd_ctx->queues[i].rq_data_on_host &&
			thd_ctx->queues[i].rqd_mkey && flexio_device_mkey_destroy(thd_ctx->queues[i].rqd_mkey)) {
...
static int clean_up_app_sq(struct app_context* app_ctx, struct thread_context *thd_ctx)
{
	...
	for (int i = 0; i < thd_ctx->num_queues; i++) {
		...
		if (!thd_ctx->queues[i].sq_data_on_host &&
			...
			thd_ctx->queues[i].sqd_mkey && flexio_device_mkey_destroy(thd_ctx->queues[i].sqd_mkey)) {
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < scheduler_num; i++) {
		...
		sch_ctx[i].num_queues = tenant_per_scheduler;
	}
...
#define THREAD_RUNNING_BITMAP 32
	...
	for (int i = 0; i < scheduler_num; i++) {
		...
		if (alloc_context_host_memory(&app_ctx, &sch_ctx[i], 0)) {
			printf("Failed to allocate scheduler host memory\n");
			return -1;
		}
		sch_ctx[i].thd_id = i;
		...
		for (int j = 0; j < tenant_per_scheduler; j++) {
			uint64_t cur_dmac = DMAC + i * tenant_per_scheduler + j;
			...
			struct mlx5dv_devx_obj *rq_tir_obj = flexio_rq_get_tir(sch_ctx[i].queues[j].flexio_rq_ptr);
			if (rq_tir_obj == NULL) {
				printf("Fail creating rq_tir_obj (errno %d)\n", errno);
				goto cleanup;
			}

			sch_ctx[i].queues[j].rq_action = mlx5dv_dr_action_create_dest_devx_tir(rq_tir_obj);
			if (sch_ctx[i].queues[j].rq_action == NULL) {
				printf("Fail creating rq_action (errno %d)\n", errno);
				goto cleanup;
			}

			sch_ctx[i].queues[j].rx_dr_rule = mlx5dv_dr_rule_create(rx_dr_matcher_root, match_value, 1, &sch_ctx[i].queues[j].rq_action);
			if (sch_ctx[i].queues[j].rx_dr_rule == NULL) {
			...
			sch_ctx[i].queues[j].tx_action = mlx5dv_dr_action_create_dest_table(tx_dr_table_sws);
			if (!sch_ctx[i].queues[j].tx_action) {
				printf("Failed creating dest SWS table action (errno %d).\n", errno);
				goto cleanup;
			}

			sch_ctx[i].queues[j].tx_dr_rule = mlx5dv_dr_rule_create(tx_dr_matcher_root, match_value, 1, &sch_ctx[i].queues[j].tx_action);
			if (!sch_ctx[i].queues[j].tx_dr_rule) {
				printf("Fail creating dr_rule (errno %d).\n", errno);
				goto cleanup;
			}

			sch_ctx[i].queues[j].tx_action2 = mlx5dv_dr_action_create_dest_vport(tx_dr, 0xFFFF);
			if (!sch_ctx[i].queues[j].tx_action2) {
				printf("Failed creating dest SWS table action (errno %d).\n", errno);
				goto cleanup;
			}

			sch_ctx[i].queues[j].tx_dr_rule2 = mlx5dv_dr_rule_create(tx_dr_matcher_sws, match_value, 1, &sch_ctx[i].queues[j].tx_action2);
			if (!sch_ctx[i].queues[j].tx_dr_rule2) {
				printf("Fail creating dr_rule (errno %d).\n", errno);
				goto cleanup;
			}
	...
	for (int i = 0; i < threads_num; i++) {
		...
		struct mlx5dv_flow_match_parameters *match_value = 0;
		int match_value_size;
		uint64_t cur_dmac = DMAC + scheduler_num * tenant_per_scheduler + i;
		// uint64_t cur_dmac = DMAC;
		printf("%lx\n", cur_dmac);

		handler_attr.host_stub_func = flexio_pp_dev_1;
		...
		if (alloc_context_host_memory(&app_ctx, &thd_ctx[i], NVME_QUEUE_MEMORY_SIZE)) {
			printf("Failed to allocate worker host memory\n");
			return -1;
		}
		...
		thd_ctx[i].rq_tir_obj = flexio_rq_get_tir(thd_ctx[i].queues->flexio_rq_ptr);
...
cleanup:
	...
	for (size_t i = 0; i < scheduler_num; i++) { 
		...
		/* Clean up rx rule if created */
			if (sch_ctx[i].queues[j].rx_dr_rule) {
				if (mlx5dv_dr_rule_destroy(sch_ctx[i].queues[j].rx_dr_rule)) {
					printf("Failed to destroy rx rule\n");
				}
			}

			if (sch_ctx[i].queues[j].rq_action) {
				if (mlx5dv_dr_action_destroy(sch_ctx[i].queues[j].rq_action)) {
					printf("Failed to destroy rx action\n");
				}
			}
		}
	}
	...
	for (size_t i = 0; i < scheduler_num; i++) {
		for (size_t j = 0; j < tenant_per_scheduler; j++) {
			/* Clean up rx rule if created */
			if (sch_ctx[i].queues[j].tx_dr_rule2) {
				if (mlx5dv_dr_rule_destroy(sch_ctx[i].queues[j].tx_dr_rule2)) {
					printf("Failed to destroy tx rule\n");
				}
			}
			if (sch_ctx[i].queues[j].tx_action2) {
				if (mlx5dv_dr_action_destroy(sch_ctx[i].queues[j].tx_action2)) {
					printf("Failed to destroy tx rule\n");
				}
			}
			if (sch_ctx[i].queues[j].tx_dr_rule) {
				if (mlx5dv_dr_rule_destroy(sch_ctx[i].queues[j].tx_dr_rule)) {
					printf("Failed to destroy tx root rule\n");
				}
			}
			if (sch_ctx[i].queues[j].tx_action) {
				if (mlx5dv_dr_action_destroy(sch_ctx[i].queues[j].tx_action)) {
					printf("Failed to destroy tx root rule\n");
				}
			}
		}
	...
	for (size_t i = 0; i < threads_num; i++) {
		...
		if (thd_ctx[i].mr && ibv_dereg_mr(thd_ctx[i].mr)) {
			printf("Failed to deregister worker MR\n");
		}
		if (thd_ctx[i].host_alloc_base && munmap(thd_ctx[i].host_alloc_base, thd_ctx[i].host_alloc_size)) {
			printf("Failed to unmap worker host buffer\n");
		}
	}
	...
	for (size_t i = 0; i < scheduler_num; i++) {
		...
		if (sch_ctx[i].mr && ibv_dereg_mr(sch_ctx[i].mr)) {
			printf("Failed to deregister scheduler MR\n");
		}
		if (sch_ctx[i].host_alloc_base && munmap(sch_ctx[i].host_alloc_base, sch_ctx[i].host_alloc_size)) {
			printf("Failed to unmap scheduler host buffer\n");
		}
	}
