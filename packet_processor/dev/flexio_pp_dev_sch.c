...
sch_ctx_init(struct flexio_dev_thread_ctx *dtctx,
  ...
  for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
    dpa_schs_ctx[i].queues[j].sq_lkey =
        data_from_host->queues[j].sq_transf.wqd_mkey_id;
    dpa_schs_ctx[i].queues[j].rq_lkey =
  ...
  for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
    for (uint64_t sq_pi = 0; sq_pi < (1UL << LOG_Q_DEPTH); sq_pi++) {
      ...
      flexio_dev_swqe_seg_ctrl_set(
          swqe, sq_pi, dpa_schs_ctx[i].queues[j].sq_ctx.sq_number,
      ...
      flexio_dev_swqe_seg_mem_ptr_data_set(
          swqe, 0, dpa_schs_ctx[i].queues[j].sq_lkey, 0);
  ...
  size_t base_cycle_budget =
      SCHED_PERIOD_CYCLES_PERCENT * 82 * threads_num_per_scheduler;
  // size_t base_cycle_budget = 1530000;
  if (tenants_num > 0 && sum_weight > 0) {
    for (uint32_t t = 0; t < tenants_num; t++) {
      dpa_schs_ctx[i].tenant_cycle_target[t] =
          base_cycle_budget * cycle_weights[t] / sum_weight;
    }
  } else {
    for (uint32_t t = 0; t < tenants_num; t++) {
  ...
  for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
    ...
    for (uint32_t t = 0; t < tenants_num; t++) {
      __atomic_store_n(&offload_info[thd_id].busy_cycle[t], 0,
                       __ATOMIC_RELAXED);
    }
  }
  // dpa_schs_ctx[i].rq_ctx.rqd_dpa_addr =
  // data_from_host->queues[j].rq_transf.wqd_daddr;
  // dpa_schs_ctx[i].sq_ctx.sqd_dpa_addr =
  // data_from_host->queues[j].sq_transf.wqd_daddr;
  flexio_dev_status_t ret;
  ret = flexio_dev_window_config(dtctx, (uint16_t)dpa_schs_ctx[i].window_id,
                                 data_from_host->result_buffer_mkey_id);
...
__dpa_global__ void flexio_scheduler_handle(uint64_t thread_arg) {
  ...
  for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
    uint32_t thd_id = i * data_from_host->num_queues + j;
    if (__atomic_load_n(&offload_info[thd_id].status, __ATOMIC_ACQUIRE) ==
            EU_OFF &&
  ...
  register size_t reschedule_cycle =
      __dpa_thread_cycles() + time_interval * DPA_FREQ_HZ;

  /* 1ms scheduling period */
  register size_t sched_period_cycles = SCHED_PERIOD_CYCLES;
  register size_t next_sched_cycle =
      __dpa_thread_cycles() + sched_period_cycles;

#if report_cycle_usage
  register size_t report_interval_cycles = DPA_FREQ_HZ; /* 1s */
  register size_t next_report_cycle =
      __dpa_thread_cycles() + report_interval_cycles;
#endif
  ...
  for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
    ...
    for (uint32_t t = 0; t < tenants_num; t++) {
      __atomic_store_n(&offload_info[thd_id].busy_cycle[t], 0,
                       __ATOMIC_RELEASE);
      __atomic_store_n(&offload_info[thd_id].restrict_tenant[t], 0,
                       __ATOMIC_RELEASE);
    }
  ...
  while (now_cycle < reschedule_cycle) {
    ...
    for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
      ...
      eu_status current_status =
          __atomic_load_n(&offload_info[thd_id].status, __ATOMIC_ACQUIRE);

      if (current_status == EU_OFF) {
        flexio_dev_msix_send(dtctx, dpa_thds_ctx[thd_id].rq_cq_ctx.cq_number);
        __atomic_store_n(&offload_info[thd_id].status, EU_HANG,
                         __ATOMIC_RELEASE);
    ...
    for (uint32_t t = 0; t < tenants_num; t++) {
      ...
      for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
        uint32_t thd_id = i * data_from_host->num_queues + j;
        current_used += __atomic_load_n(&offload_info[thd_id].busy_cycle[t],
                                        __ATOMIC_ACQUIRE);
      }
      if (current_used >= this_sch_ctx->tenant_cycle_target[t]) {
        for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
          uint32_t thd_id = i * data_from_host->num_queues + j;
          __atomic_store_n(&offload_info[thd_id].restrict_tenant[t], 1,
                           __ATOMIC_RELEASE);
        }
      }
    }
    ...
    if (now_cycle >= next_sched_cycle) {
      for (uint32_t t = 0; t < tenants_num; t++) {
        ...
        for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
          uint32_t thd_id = i * data_from_host->num_queues + j;
          size_t thd_cycles = __atomic_exchange_n(
              &offload_info[thd_id].busy_cycle[t], 0, __ATOMIC_ACQ_REL);
          __atomic_store_n(&offload_info[thd_id].restrict_tenant[t], 0,
                           __ATOMIC_RELEASE);
  ...
  for (uint32_t j = 0; j < data_from_host->num_queues; j++) {
    ...
    flexio_dev_cq_arm(dtctx, this_tenant->rq_cq_ctx.cq_idx,
                      this_tenant->rq_cq_ctx.cq_number);
  }
