...
int pp_queue(struct flexio_dev_thread_ctx *dtctx, struct dpa_thread_context* this_thd_ctx __unused,
	     struct flexio_dpa_dev_queue* tenant, int thd_id __unused, int tenant_id, uint32_t *result)
{
...
__dpa_rpc__ uint64_t thd_ctx_init(uint64_t data)
{
	...
	if (data_from_host->sq_transf.wq_num && data_from_host->sq_transf.wq_ring_daddr) {
		/* Set context for SQ */
		...
		memset(&(dpa_thds_ctx[i].sq_ctx), 0, sizeof(dpa_thds_ctx[i].sq_ctx));
		memset(&(dpa_thds_ctx[i].sq_cq_ctx), 0, sizeof(dpa_thds_ctx[i].sq_cq_ctx));
		memset(&(dpa_thds_ctx[i].dt_ctx), 0, sizeof(dpa_thds_ctx[i].dt_ctx));
	}
